...
def import_workspace(data: dict[str, Any]) -> dict[str, int]:
    ...
    with database.get_connection() as conn:
        ...
        for m in data.get("messages", []):
            tc = (
                json.dumps(m.get("tool_calls", [])) if not isinstance(m.get("tool_calls"), str) else m.get("tool_calls")
            )
            att = (
                json.dumps(m.get("attachments", []))
                if not isinstance(m.get("attachments"), str)
                else m.get("attachments")
            )
            conn.execute(
                """
