...
def test_audit():
    ...
    proj_res = request_json("/api/projects", method="POST", data={
    ...
    up_res = request_json(f"/api/projects/{proj_id}", method="PATCH", data={
        "system_instructions": "Updated custom instructions for project."
    })
    ...
    c1_res = request_json("/api/conversations", method="POST", data={
        "title": "Initial Audit Chat",
        "project_id": None,
        "model": "qwen2.5:32b"
    })
    ...
    unarchived_list = request_json(f"/api/conversations")
    assert not any(c["id"] == c1_id for c in unarchived_list["conversations"]), "Archived conversation should not appear in unarchived list"
    archived_list = request_json(f"/api/conversations?include_archived=true")
    assert any(c["id"] == c1_id for c in archived_list["conversations"]), "Archived conversation should appear when include_archived=true"
    ...
    unarchived_list2 = request_json(f"/api/conversations")
    ...
    print(f"  [OK] Forked/Branched conversation successfully created (ID: {branch_id}) containing exact history up to target message turn")
    ...
    print(f"  [OK] Exported workspace: {len(export_res['projects'])} project(s), {len(export_res['conversations'])} conversation(s), {len(export_res['messages'])} message(s)")
